...
interface EditNourishModalProps {
  open: boolean;
  item?: NourishItem;
  onClose: () => void;
...
export function EditNourishModal({ open, item, onClose, onSubmit }: EditNourishModalProps) {
  ...
  useEffect(() => {
    if (item) {
      ...
      Promise.resolve().then(() => setThumbnailUrl(item.thumbnailUrl || ''));
    } else {
      reset();
      Promise.resolve().then(() => setThumbnailUrl(''));
    }
  }, [item, setValue, reset]);
  ...
  return (
    <Modal
      title={<span className="text-2xl font-bold text-gray-900">Edit nourish</span>}
      open={open}
      ...
      <Form layout="vertical" onFinish={handleSubmit(handleFormSubmit)} className="mt-4">
        ...
        <Controller
          ...
          render={({ field }) => (
            ...
            >
              <Input {...field} placeholder="Heading" className="rounded-lg" />
            </Form.Item>
          )}
        />
        ...
        <Controller
          ...
          render={({ field }) => (
            ...
            >
              <Input.TextArea {...field} rows={5} className="rounded-lg" />
            </Form.Item>
          )}
        />
