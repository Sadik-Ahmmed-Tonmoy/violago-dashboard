...
interface CreateJourneyLessonModalProps {
  open: boolean;
  title: string;
  partId: string;
  onClose: () => void;
...
export function CreateJourneyLessonModal({
  open,
  title,
  partId,
  onClose,
  onSubmit,
}: CreateJourneyLessonModalProps) {
  ...
  return (
    <Modal title={title} open={open} onCancel={onClose} footer={null} width={600} destroyOnClose centered>
      <Form form={form} layout="vertical" onFinish={handleFinish}>
        <Form.Item name="title" label="Lesson Title" rules={[{ required: true, message: 'Title is required' }]}>
          <Input placeholder="e.g., Crate Training" />
        </Form.Item>
        ...
        <Form.Item label="Image (optional)">
          ...
          {previewUrl && (
            <div className="mt-2 flex justify-end">
              <button onClick={() => { URL.revokeObjectURL(previewUrl); setFile(null); setPreviewUrl(''); }} className="text-xs text-red-500">Remove</button>
            </div>
          )}
        </Form.Item>
